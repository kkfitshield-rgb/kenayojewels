...
export const handleInquiry: RequestHandler = async (req, res) => {
  try {
    ...
    try {
      ...
      console.error(
        `Failed to send owner notification for inquiry ${inquiryId}:`,
        emailError,
      );
    }
    ...
    try {
      customerEmailSent =
        await emailService.sendCustomerConfirmation(inquiryData);
      if (customerEmailSent) {
        console.log(
          `Customer confirmation email sent for inquiry ${inquiryId}`,
        );
      }
    } catch (emailError) {
      console.error(
        `Failed to send customer confirmation for inquiry ${inquiryId}:`,
        emailError,
      );
    }
